...
**Warehouse:** DuckDB `fuel_prices` (`python scripts/build_duckdb.py` → `data/energy.duckdb`) — same grain as the CSV in GitHub  
...
Ingest the official CSV/API into `prices_raw`, snapshot into DuckDB (or Postgres/BigQuery at work), point Power BI at that table, and keep the five SQL files as the regression suite for schema changes.
